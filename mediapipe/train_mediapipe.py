# train_mediapipe.py (V2: 단일 뷰, 양방향)
# -----------------------------------------------------------------------------
# [목표]: "단일 뷰" NPY 데이터로 양방향(Bidirectional) Encoder를 학습시킵니다.
#         실시간 추론(1개 카메라)을 위한 V2 모델입니다.
# [입력]: 'all_index_npy.csv'
# -----------------------------------------------------------------------------
...
class SignLanguageDataset_NPY(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        npy_path = item['npy_path'] # .npy 파일 경로
        ...
        try:
            ...
            final_sequence = np.zeros((self.max_len, INPUT_SIZE_NPY), dtype=np.float32) 
...
if __name__ == '__main__':
    ...
    # (V2 모델 크기 - 1340개 데이터에 맞춘 추천값)
    HIDDEN_SIZE = 768 
    NUM_LAYERS = 3   
    EMBED_SIZE = 256
    DROPOUT_PROB = 0.7 # (과적합 방지)
    
    NUM_WORKERS = 4 
    PATIENCE = 10 
    
    # [V2] 단일 뷰 NPY 인덱스 파일 경로
    ALL_IN_ONE_NPY_CSV = 'all_index_npy.csv'

    # --- 데이터 준비 ---
    print(f"[V2] 단일 뷰 데이터 인덱스 로딩 ({ALL_IN_ONE_NPY_CSV})...")
    try:
        all_df = pd.read_csv(ALL_IN_ONE_NPY_CSV) 
    except FileNotFoundError:
        print(f"[오류] '{ALL_IN_ONE_NPY_CSV}' 파일을 찾을 수 없습니다.")
        print(">>> 'preprocess_to_npy.py (V2)'를 먼저 실행해야 합니다.")
        exit()
    ...
    patience_counter = 0
    best_model_path = 'model_a_mediapipe_best.pth' # (V2 모델 이름)

    # --- 모델 학습 루프 ---
    print(f"\n[V2] Seq2Seq (단일 뷰 NPY) 모델 학습을 시작합니다... (DEVICE: {DEVICE})")
